# 단일 스테이지를 멀티스테이지로 — 사라진 ctypes와 CoW 실측

지난 글에서는 CUDA 베이스 이미지 위에 `train_mnist.py`를 얹는 Dockerfile을 완성했다. 빌드 에러 4개를 순서대로 잡고 나니 이미지는 잘 도는데, 문제는 크기였다. `docker images`로 확인한 크기가 3.22GB. 이번 글의 목표는 그 이미지를 멀티스테이지로 다이어트시키는 것이었는데, 실제로는 다이어트보다 두 가지 사고를 치고 수습하는 데 시간을 더 썼다. 그 과정에서 venv COPY가 실제로 뭘 옮기고 뭘 못 옮기는지, 그리고 "빌드했다"와 "검증했다"가 왜 다른 말인지를 몸으로 확인했다.
...
ctypes 문제를 고치면서 Dockerfile의 torch 버전 핀도 같이 손봤다. 처음 써둔 값은 외부 예제에서 그대로 가져온 `torch==2.5.1` / `torchvision==0.20.1` 조합이었는데, 이건 로컬 venv에서 실측한 버전(torch 2.12.1, torchvision 0.27.1)과 맞지 않았다. 로컬 버전 확인 없이 남의 예제 숫자를 그대로 옮겨 쓴 것이니 정직하게 인정할 부분이다. 로컬 실측값 기준으로 핀을 고쳤다.

핀을 고치고 빌드하고 실행까지 했는데, 이상하게 이미지 안의 torch는 여전히 2.5.1이었다. 파일을 분명히 고쳤는데 왜 옛날 값이 나오나 한참 헤맸는데, 원인은 단순했다. **파일을 저장(13:01)하기 전에 빌드(12:58)를 돌렸던 것**이다. 시간을 대조해보고서야 알았다.

이 해프닝 이후로 확인 방법을 바꿨다. "빌드했다"는 선언이 아니라, 만들어진 이미지 안을 직접 열어서 확인하는 것.

```bash
docker run --rm <이미지> python -c "import torch; print(torch.__version__)"
```

이렇게 컨테이너를 띄워 이미지 내부에서 직접 버전을 찍어보고 나서야 `2.12.1+cpu`를 확인할 수 있었다. 빌드 로그가 성공으로 끝났다는 것과, 그 이미지 안에 내가 원하는 버전이 실제로 들어 있다는 것은 별개의 사실이다.

## 크기 실측 — 레이어 단위로 뜯어보기

두 사고를 수습하고 나서 원래 목표였던 크기 비교를 했다.
