...
## 준비할 이미지
1. `1. wifi-off-traceback.png` — Wi-Fi 끄고 실행했을 때의 traceback (shot: 직접 실행 화면 캡처)
2. `2. eacces-exit0-terminal.png` — 권한 제거 후 실행 + `echo $?`로 exit code 확인 (shot: 직접 실행 화면 캡처)
...
<!-- IMG: 1 | wifi-off-traceback | shot | Wi-Fi 차단 후 실행한 traceback(gaierror→URLError→RuntimeError) 터미널 캡처 — 본인 실험 결과라 웹 수집 불가 -->
<!-- IMG: 2 | eacces-exit0-terminal | shot | 권한 제거 후 실행 + echo $? 가 0을 찍은 터미널 캡처 — 본인 실험 결과라 웹 수집 불가 -->
