...
인프라 엔지니어가 알아야 할 CS 기초·리눅스 커맨드·커널 지식의 한국어 학습 KB. `Claude/`·`AI-Infra/`와 동일 패턴(노트 + MOC + lint)을 따른다. [[30 AI Infrastructure]](GPU 서빙·K8s)의 **토대 레이어** — 컨테이너도 GPU 서버도 결국 리눅스 위에서 돈다.
...
## 핵심 명제
인프라 역량의 본질은 커맨드 암기가 아니라 **"커널이 왜 그렇게 동작하는지"를 알고 증상에서 원인으로 내려가는 능력**이다. 개념(01~06) 없이 도구(07~08)만 알면 스트리트라이트 안티패턴에 빠진다. 이 KB를 마치면 [[30 AI Infrastructure]]의 GPU·K8s 레이어가 "리눅스의 응용"으로 읽힌다.
