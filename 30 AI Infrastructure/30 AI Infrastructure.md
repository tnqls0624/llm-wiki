...
## 선행 지식 (Infra 토대)
GPU 서버도 컨테이너도 결국 리눅스 위에서 돈다 — [[20 Architecture]] KB가 이 KB의 토대 레이어다. 특히 [[02 메모리 관리]](KV cache·PagedAttention의 바탕), [[06 컨테이너 내부 구조]](cgroups·GPU 디바이스 주입), [[08 성능 분석과 트러블슈팅]](서빙 p99 디버깅)을 먼저 알면 아래 3영역이 "리눅스의 응용"으로 읽힌다.
