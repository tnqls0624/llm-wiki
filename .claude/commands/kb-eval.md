...
**종료 코드는 3원화되어 있다** — `0` 통과 · `1` 실패 · `2` **판정 불가**. `ok`는 각각 `true`/`false`/`null`이다.

`2`가 따로 있는 이유: v3 초판은 `undecidable`을 `rc=0`으로 냈고, 그래서 **전량을 `skipped`로 제출하면 아무것도 채점하지 않고 "통과"**했다(자체 감사에서 재현 — v2의 체리피킹보다 나쁘다, 정답 지식이 전혀 필요 없다). 판정하지 못한 것은 통과가 아니다. 그렇다고 실패로 만들면 v2의 영구 실패 트랩이 돌아오므로 상태를 분리했다.
...
3. **routing balanced accuracy ≤ 0.5** — 클래스별 recall의 평균. **상수 전략(항상 한 클래스)은 표본 크기·불균형과 무관하게 정확히 0.5**이므로 확실히 잡힌다.
4. **원장 손상** — 읽을 수 없는 줄이 있으면 부분 이력으로 판정하지 않는다.

**판정 불가(`rc=2`)로 보는 것:**
- **커버리지 미달** — 코호트에서 실제 채점된 비율이 50% 미만. `skipped`로 코호트를 채우는 것은 채점이 아니다.
- **소수 클래스 표본 부족** — balanced accuracy가 0.5를 넘어도, 클래스별 채점 수가 **각 2건 미만**이면 통과를 주장하지 않는다. 소수 클래스가 1건이면 그 한 건의 답이 BA를 0.5↔1.0으로 흔든다(9 queue / 1 drop에서 `drop` 하나만 맞히면 나머지 9개가 상수여도 BA=1.0). 상수 전략은 위 3번에서 이미 잡히므로 여기서 보류해도 게이트가 약해지지 않는다.
- **정답이 한 클래스뿐** — BA를 계산할 수 없다.
- **큐에서 라벨 소실**, 채점 이력 없음.
- **시도한 축이 하나라도 미판정** — grounding이 통과했다는 이유로 `rc=0`을 내면, routing에 대해 아무것도 말할 수 없는 상태가 "다 괜찮다"로 읽힌다. `attempted_axes`·`decided_axes`·`unresolved_axes`가 출력에 함께 나온다.

은퇴 케이스와 고아 행(원장에만 있는 id)은 게이트를 막지 않는다.

> **지금 이 vault의 상태**: routing 골든셋이 9 queue / 1 drop이므로 이 축은 **구조적으로 `rc=2`**다. 상수 전략은 잡히지만 통과를 주장할 수는 없다. `/claude-radar review`에서 `[dismissed]` 결정이 최소 2건 쌓여야 routing이 통과 판정을 낼 수 있다.
