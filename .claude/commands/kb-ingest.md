...
## 5. 갱신 의무
- `.claude/rules/vault-rules.md`의 "Update duty (CANONICAL — 3 steps)"를 수행한다(정본 참조 — 단계·수치는 여기서 재명세하지 않는다). MOC는 만진 디렉토리의 허브(`80 Tooling/80 Tooling.md` 또는 새 주제의 `<주제>/<주제>.md`)를 갱신한다.
- **새 주제 디렉토리를 만들었다면** `hot.md`의 `## Vault state`에도 그 디렉토리를 추가한다(KB가 더 이상 `80 Tooling/`만이 아님을 boot context에 반영).
