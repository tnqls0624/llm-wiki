...
GeekNews·GitHub·Hacker News·Anthropic 공식·dev.to·npm 등에서 Claude Code 활용 관련 신규 정보를 모아, "우리 `.claude/` 프레임워크에 더하면 좋을 것(skill/agent/command/rule)"과 "`80 Tooling/` KB에 박제할 지식"을 추천 큐에 쌓는다. **실제 생성·박제는 사용자 동의 후 `review` 모드에서만** 한다.
...
- **kb-ingest** — 개념·사용법 지식으로 KB에 박제할 가치가 있는 자료. **박제 위치는 source로 라우팅**한다: source가 `AI-infra:`로 시작하면 **AI 인프라 학습 토픽**이므로 `30 AI Infrastructure/` KB로, 그 외 Claude Code 관련은 `80 Tooling/` KB로. (제안에 어느 KB인지 명시.)
...
- **kb-ingest** → 해당 url을 `/kb-ingest` 절차로 KB에 박제한다(스크럽 → 주제 판별 → 노트 생성/갱신 → 갱신 의무 3단계). 박제 위치: `AI-infra:` 소스는 `30 AI Infrastructure/`(MOC는 `30 AI Infrastructure/30 AI Infrastructure.md`), 그 외는 `80 Tooling/`. 분량이 크면 `kb-updater` 서브에이전트(Agent tool, `subagent_type: kb-updater`)에 위임한다.
