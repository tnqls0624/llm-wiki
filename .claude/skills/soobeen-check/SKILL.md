...
| ⑤ 계획 | "다음에 할 것" 칸을 본인이 마감했나 | 빈칸/"너가 적어줘"류 방치. AI가 정한 계획을 본인이 확인·수용해 옮긴 것은 미달 아님(2026-07-09 본인 확인) |
...
## 4. KB 승격 후보 — 최대 1개 (2026-08-23, 아키텍처 승격 루프)
표 아래에 **승격 후보를 최대 1개만** 제시한다. 근거: 커리어 KB(`20 Architecture/`·`30 AI Infrastructure/`)의 유일한 유입 경로가 이 이벤트다 — 시간 트리거 회고는 이 사용자에게서 실측 사망(Weekly Review 0건)했고, 살아 있는 갱신 경로(kb-sync)는 도구 문서만 밀어 `80 Tooling/` 편중을 만들었다(현재 비율은 하드코딩하지 않는다 — hot.md Content 줄이 정본. 63.3%가 박혀 있던 동안 hot.md는 64.0%로 갱신돼 모순이 생겼다).
- 후보 선정: 오늘 검토 로그·log.md에서 **완료 체크됐는데 20/30에 대응 노트가 없는 학습 항목** 하나. 없으면 "승격 후보 없음" 한 줄 — **밀린 목록을 만들지 않는다**(처리 용량 존중, 후보는 항상 0~1개).
- 형식(빈칸 템플릿 — 완성 문장 금지): `승격 후보: ___ → 30 AI Infrastructure/NN ___.md — 핵심 주장 한 줄: ___`
- **이 스킬은 보고만 한다** — 노트 생성은 사용자가 수락하면 메인 세션이 `/kb-save`로 수행(이 스킬의 "파일 수정·커밋하지 않는다" 계약은 그대로다).
