...
표 아래에 **승격 후보를 최대 1개만** 제시한다. 근거: 커리어 KB(`20 Architecture/`·`30 AI Infrastructure/`)의 유일한 유입 경로가 이 이벤트다 — 시간 트리거 회고는 이 사용자에게서 실측 사망(Weekly Review 0건)했고, 살아 있는 갱신 경로(kb-sync)는 도구 문서만 밀어 63.3% 편중을 만들었다.
