<!-- generated by study-brief.py | date=2026-07-24 -->
# 오늘의 학습 — 2026-07-24 (금)

> 어제: 새 산출물 없음 → 진도 W3 D1 동결 (7-16 이후 8일 커밋 공백). ⚠️ 미커밋 `docs/log.md`는 포매팅 churn — `apt ... &amp;&amp;` escape 손상 있음, 커밋 전 `git checkout docs/log.md`로 되돌리거나 정정할 것.

## 오늘 할 것 (평일 · 20~40분)
**W3** · D2: NVIDIA Container Toolkit 동작 원리 — `--gpus all`이 하는 일

### 학습 가이드
- 🎯 개념: nvidia-container-runtime이 컨테이너 시작 시점에 드라이버 라이브러리·`/dev/nvidia*`를 주입하는 구조(runc hook). "이미지에 드라이버를 안 굽는" 이유가 여기서 완성됨
- ✅ 완료: log.md에 주입 흐름 요약 + Block 2 D1에서 실측할 체크리스트 3개

> 피곤하면 10분 대안: 어제 셀/스크립트 1개 다시 실행 + 변수 1개 바꿔 결과 비교 후 커밋. 연속성만 유지.

## 어제 한 것 검토
`/study-coach review` 실행 → 어제 산출물(`/Users/leesoobeen/Desktop/Project/ai-infra-lab`의 커밋·코드·docs/log.md)을 점검하고 진도를 체크합니다.

## 세션 마감 형식
```
오늘 한 것:
발생한 문제:
해결하거나 확인한 내용:
다음에 할 것:
```
