...
if not fp or not norm.endswith(".md"):
    sys.exit(0)
if any(seg in norm for seg in ("/.claude/", "/.agents/", "/.codex/", "/.obsidian/", "/.space/", "/.git/", "/.trash/", "/Projects/", "/blog/")):
    sys.exit(0)  # Projects/=운영 워크스페이스, blog/=블로그 초안+수집 이미지 — KB 노트 아님, 배치 린터 EXCLUDE와 일치
...
for p in glob.glob(os.path.join(vault_root, "**", "*.md"), recursive=True):
    pn = p.replace("\\", "/")
    if any(seg in pn for seg in ("/.claude/", "/.agents/", "/.codex/", "/.obsidian/", "/.space/", "/.git/", "/.trash/", "/Projects/", "/blog/")):
        continue
