...
- Content lives in **topic directories**, each `<Topic>/` holding notes + a `<Topic>/<Topic>.md` MOC hub. Current topics: `80 Tooling/` (Claude Code official-docs KB 30 notes + the absorbed vault self-diagnosis note) · `20 Architecture/` (CS/Linux/kernel fundamentals, 10) · `30 AI Infrastructure/` (backend→AI-infra learning, 8). 52 `.md` = **49 notes + 3 MOC hubs** — when quoting ratios, state which denominator you used; kb-lint counts 52 (it does not subtract MOCs). This is **content (data)**.
