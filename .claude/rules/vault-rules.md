...
- Content lives in **topic directories**, each `<Topic>/` holding notes + a `<Topic>/<Topic>.md` MOC hub. Current topics: `80 Tooling/` (Claude Code official-docs KB, 29 notes) · `30 AI Infrastructure/` (backend→AI-infra learning, 8 notes) · `20 Architecture/` (CS/Linux/kernel fundamentals, 10 notes) · `80 Tooling/` (vault's own architecture self-diagnosis). This is **content (data)**.
