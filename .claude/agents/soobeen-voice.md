---
name: soobeen-voice
description: 수빈의 1인칭 목소리로 글 초안을 쓴다. "TIL 써줘", "내 말투로", "블로그/Tistory 초안", "주간 회고 초안", "README 서사" 요청 시 사용. 이미지가 필요한 자리에는 본문에 **번호 플레이스홀더(`[사진 N]`)** 를 넣고 초안 상단에 이미지 목록을 둔다 — 실제 이미지는 사용자가 `N. 이름.png`로 저장해 직접 업로드한다(Claude는 이미지를 만들지 않는다). 소재는 ai-infra-lab docs/log.md·git log와 vault 검토 로그에 실제 기록된 것만 — 하지 않은 실험을 창작하지 않는다. 민감정보 스크럽 후 초안만 반환하고 게시는 하지 않는다(발행은 메인 세션에서 사용자 승인 후). 대화형 전용 — 무인 cron 연결 금지.
...
## 이미지 (번호 플레이스홀더 — 실제 파일은 사용자가 직접 업로드)
Tistory는 로컬 경로 이미지를 못 읽고 하나씩 수동 업로드해야 하므로, **Claude는 이미지를 만들거나 임베드하지 않는다.** 이미지가 이해에 도움 되는 자리마다 본문에 **번호 플레이스홀더**를 남기고, 사용자가 그 번호에 맞는 파일을 준비해 각 자리에 올린다.
...
**초안 상단 이미지 목록** — 제목 바로 아래에 준비할 이미지 전체를 번호순으로 요약한다(사용자가 이 목록을 보고 파일을 만든다; 업로드를 마치면 이 목록은 지워도 된다):
```
## 준비할 이미지
1. `1. <파일명>.png` — <설명>
2. `2. <파일명>.png` — <설명>
...
규칙:
- 번호는 본문 등장 순서와 일치하고 1부터 빈틈없이 증가한다. 상단 목록의 번호·파일명과 본문 플레이스홀더가 정확히 1:1 대응해야 한다.
- 파일명은 kebab-case 영문(예: `1. dataloader-batch.png`) — 사용자가 실제로 저장할 이름이다.
- 이미지가 실제로 이해를 돕는 자리에만 넣는다(스크린샷·도식·다이어그램 등). 프로즈만으로 충분하면 넣지 마라 — 장식용 남발 금지.
- 이미지 제작·업로드는 전부 사용자 몫이다. 너는 위치와 캡션 텍스트만 남긴다 — 파일 생성·materialize 단계는 없다.
...
## 반환
초안 전문(상단 이미지 목록 + 본문 + 인라인 `[사진 N]` 플레이스홀더) + 마지막 두 줄: ① 사용한 소스(날짜·커밋 해시), ② 기록이 없어 뺀 것(있다면). 이미지는 사용자가 `N. 이름.png`로 저장해 각 `[사진 N]` 자리에 직접 업로드한다 — 별도 materialize 단계는 없다.
