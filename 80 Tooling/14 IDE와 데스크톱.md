...
area: 개발자 학습
tags: [claude-code]
source_urls: [vs-code, jetbrains, desktop, desktop-quickstart, platforms, desktop-linux, desktop-wsl, desktop-ios-simulator]
notion_url: ""
...
Claude Code를 터미널 밖에서 — VS Code/Cursor 확장, JetBrains 플러그인, 그리고 독립 데스크톱 앱(GUI)에서 — 사용하는 방법을 정리한다. 모든 표면(surface)은 동일한 엔진을 공유하며 CLAUDE.md·MCP·설정 파일을 같이 읽는다. 어떤 표면을 고를지에 대한 비교는 맨 아래 [[#플랫폼과 통합 (어디서 실행할지 고르기)]] 참고. 허브는 [[80 Tooling]].
